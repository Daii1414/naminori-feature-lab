...
- **What is this?** A high-performance content script that parses YouTube comment threads, hides those lacking Japanese Kana scripts, and provides an instant toggle button to reveal hidden comments.
- **What files matter?** `content.js` (comment traversal, script heuristics, CPU-throttled MutationObserver, button lifecycle) and `content.css` (zero-cost hiding rules & button styling).
- **What must be preserved?** The `isJapaneseText` RegEx heuristic, the `requestAnimationFrame` execution throttle, the mutation observer scoping (ignoring video player ticks and button mutations), the `ntCommentText` title cache for SPA stability, and the body class toggle pattern for instant zero-lag reveal.
...
- Employs a strict **`requestAnimationFrame` throttle** to batch hundreds of YouTube DOM mutations into a single 60 FPS frame update.
- Isolates DOM mutations to `#comments`, completely ignoring the high-frequency ticks of YouTube's HTML5 video player.
...
- [ ] **Performance Stability:** CPU usage remains normal while video plays (no freezing, lag, or mutation feedback loops).
...
- [ ] Navigating to another video (SPA transition) resets the button and count without page reload.

## 6. Expected Correct Behavior
Foreign-language comments are excluded from view without any browser stutter. The user only reads authentic Japanese discourse. When requested, hidden comments can be inspected with one click.

## 7. Dependencies Used
**None (0).** Pure Vanilla JavaScript and CSS.

## 8. Chrome Permissions Used
- `host_permissions`: `*://*.youtube.com/*` (needed to run content script on YouTube video pages).

## 9. Key Files
- `manifest.json`: Injects `content.js` and `content.css`.
- `content.js`: Comment parsing, language evaluation, throttled observer, and button lifecycle management.
...
1. **Observer Scoping & Throttling (Anti-Freeze Architecture):**
   - **Do NOT observe `document.documentElement` naively without filtering.** The YouTube HTML5 video player emits timeline/progress mutations hundreds of times per second. 
   - The observer **must** check `target.closest('#comments, ytd-comments')` and discard mutations originating from the player or the extension's own Reveal button.
   - All processing **must** be throttled via `requestAnimationFrame` so that rapid comment streaming is batched into a single pass.
2. **The `isJapaneseText()` RegEx:**
...
3. **Body Class Toggle for Reveal (`body.naminori-reveal-comments`):**
   - Toggling a class on `document.body` instead of modifying hundreds of individual element classes enables instant reveal/hide of 500+ comments with zero lag.
4. **SPA Video Change Reset (`lastVideoId`):**
...
- **Mutation cascade & infinite loop:** Throttles calls and ignores video player ticks and self-injected button nodes.
- **Virtual DOM element recycling:** Tracks comment text per thread element to detect text changes without re-filtering unchanged nodes.
- **Kaomoji / emojis in comments:** Prevents English comments with Japanese emojis from passing the filter.
- **Infinite scroll:** Correctly catches and processes newly loaded comment batches.
- **Light and Dark YouTube theme adaptation:** Reveal button colors adapt automatically.

## 14. Edge Cases Not Handled
- Comments written in romaji (Latin alphabet transliteration of Japanese).

## 15. Expected User Result
A clean comment section full of natural, native Japanese discussion, providing rich reading immersion without distraction or performance drops.
